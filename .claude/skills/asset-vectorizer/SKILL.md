...
- **Toolkit de composite (los 4 scripts que matan iteraciones — ✅ CONSTRUIDO + VALIDADO 25/07 contra banco de verdad conocida; corrida documentada en [`toolkit-composite.md`](../../../docs/direccion/toolkit-composite.md)):**
  - `map-pieces.mjs <composite.png> [minArea] [alphaT]` → tabla de bboxes+centroide+área por isla (connected-components) → `<composite>_pieces.json`. Elimina la ronda de "los agentes corrigen el bbox por evidencia" (el eslabón débil: 5 corrimientos de ~500px por mapear a ojo). **✅ HECHO:** recupera los bboxes con **0px de error** (banco de 5 piezas, IoU 1.0). **Límite verificado:** separa islas DESCONECTADAS por fondo (el sheet DESARMADO, input canónico); sobre el composite ENSAMBLADO devuelve 1 blob — piezas pegadas NO se parten por silueta (color/BL-21 o SAM/BL-22). Un tiling ciego por cuadrantes NO lo reemplaza (un objeto cruza celdas; [research](../../../docs/direccion/tecnicas-mapeo-verificacion-research.md) § 1d).
  ...
  - `crop-check.mjs --ring <p.png>… | <composite.png> <manifest.json>` → **el GATE DE RECORTE del paso 2, hecho script**: (a) anillo perimetral por pieza + (b) mapa de residuo (composite − ∪ piezas, con bbox del faltante). **Cobertura = gate DURO; anillo = WARNING** (un recorte exacto y uno que se comió una parte se ven igual en el borde → recortá con margen transparente para poder verificar completitud). **✅ HECHO:** caza el buzón-sin-base (anillo `bottom(450)`) y la pieza faltante (residuo 11.6%, bbox exacto) sobre banco de verdad conocida. *(Impl: canvas exacto `resize(W,H)` ANTES de `extract` — el gotcha `density`+`extract` cae mal si no.)*
  - `check-rig.py <svg>…` → lint de riggeabilidad pre-Rive: **ids duplicados** (elemento↔gradiente) por **parseo propio, NO `xmllint --valid`** — éste exige la DTD completa de SVG 1.1 (setup pesado, contra el "cero setup"); el parseo caza la colisión sin DTD/red. + well-formedness (`xmllint --nonet`) + features Rive-incompatibles (gradientTransform/mask/filter/image/stroke-dasharray/skew/CSS) = HARD; paths anónimos + grupos de 1 hijo = warn. **✅ HECHO + ya rindió:** cazó un **`<filter>` real** (blur del haz, `stdDeviation=14`) en `entrada.svg` → rig-breaker en Rive Y efecto que debería ir por código ⚡, no horneado en el SVG.
...
- `scripts/map-pieces.mjs` — `bun map-pieces.mjs <composite.png> [minArea=200] [alphaT=16]` → bboxes+centroide+área por isla (connected-components) → `<composite>_pieces.json`. Mata el mapeo a ojo. Islas DESCONECTADAS por fondo (sheet desarmado); piezas pegadas NO se parten (BL-21/BL-22).
- `scripts/locate-piece.py` — `python3 locate-piece.py <composite.png> <piece.png>…` (venv-score) localiza cada pieza CONOCIDA dentro del composite por template matching (NCC) → `(x,y)` + `peak`. **3ª vía de mapeo: LOCALIZA sin segmentar → funciona sobre el composite ENSAMBLADO** (donde map-pieces ve 1 blob). `peak≈1.0` = recorte 1:1 (posición exacta); `peak` bajo = mismatch de escala/generación → la posición viene de otro lado.
- `scripts/crop-check.mjs` — `bun crop-check.mjs --ring <p.png>…` (anillo por pieza) **|** `bun crop-check.mjs <composite.png> <manifest.json>` (anillo + residuo). Cobertura = gate DURO, anillo = WARNING. Caza buzón-sin-base + pieza faltante con bbox. Manifest: `{"pieces":[{"file","left","top"}]}`.
- `scripts/check-rig.py` — `python3 check-rig.py <svg>…` lint de riggeabilidad pre-Rive: ids duplicados (parseo, sin DTD) + `xmllint --nonet` + features Rive-incompatibles = HARD; paths anónimos + grupos de 1 hijo = warn.

Requiere `sharp` (ya en el repo) + `bun` para los `.mjs`; `xmllint` + python3 stdlib para `check-rig.py`; y `venv-score` (reconstruíble de `requirements-score.txt`, gitignored) para `score.py`.
...
### 2026-07-25 (2) — toolkit de composite construido + validado (map-pieces / crop-check / check-rig)
- **Antes:** `score.py` existía pero los otros 3 scripts del toolkit de composite eran deuda; el mapeo seguía siendo a ojo (5 corrimientos de ~500px en la tienda) y no había gate de recorte ni de riggeabilidad como script.
- **Cambió (construidos + validados contra banco de verdad conocida — corrida [`toolkit-composite.md`](../../../docs/direccion/toolkit-composite.md)):**
  1. `scripts/map-pieces.mjs` — bboxes por connected-components; **0px de error** sobre el banco (5 piezas, IoU 1.0). Límite del composite pegado verificado y documentado (devuelve 1 blob).
  2. `scripts/crop-check.mjs` — anillo perimetral (WARNING) + mapa de residuo (gate DURO); caza buzón-sin-base (`bottom`) y pieza faltante (residuo 11.6%, bbox exacto).
  3. `scripts/check-rig.py` — ids duplicados por parseo (sin DTD) + features Rive-incompatibles; cazó un `<filter>` real en `entrada.svg`.
- **Hallazgos del run (lecciones de método):** (a) las piezas del despiece se recortaron **SIN padding** → el anillo no puede afirmar completitud; **regla: recortar con margen transparente**. (b) `entrada.svg` horneó el haz de luz como `<filter>` SVG → Rive no lo soporta Y contradice la frontera 🎨/⚡ (el haz va por código ⚡). (c) `venv-score` es **efímero** → se reconstruye de `requirements-score.txt` (ahora gitignored).
- **Por qué:** cerrar la deuda del toolkit para que la próxima corrida de vectorización de composite no repague el mapeo a ojo ni pase rig-breakers a Rive.
- **Cómo revertir:** borrar `scripts/{map-pieces.mjs,crop-check.mjs,check-rig.py}` + este entry; `git checkout <sha-previo> -- SKILL.md`.

### 2026-07-25 (3) — locate-piece (template matching, 3ª vía de mapeo) + verificaciones pre-test
- **Antes:** el mapeo tenía dos vías (connected-components sobre sheet desarmado; color/SAM parqueado) y ninguna resolvía "dónde va cada pieza en el composite ENSAMBLADO" sin ojo. El loop de verificación por-pieza no estaba probado end-to-end.
- **Cambió:** **(1)** `scripts/locate-piece.py` — template matching (NCC, `skimage.feature.match_template`): localiza cada pieza CONOCIDA en el composite SIN segmentar → resuelve el mapeo sobre el ensamblado. Validado: `peak=1.0` sobre banco de verdad conocida Y sobre material real (las piezas de `pieces/` son recortes 1:1 de `tienda_clean.png` → posición exacta). **(2)** Verificado el loop `score.py` + `overlay`: score.py distingue la norma base (dE95=10, diss=0.25) de la vieja mala (dE95=14, diss=0.42); overlay genera el diff. Matiz: el verdict binario filtra lo GRUESO; las diferencias FINAS se leen en los números crudos + el diff (división de trabajo de la Opción B).
- **Por qué:** de-riesgo antes del test de método (Alan preguntó si había otra forma de mapear sin el bbox difícil → sí, template matching, y resuelve el caso de la tienda con peak perfecto).
- **Cómo revertir:** borrar `scripts/locate-piece.py` + este entry; `git checkout <sha-previo> -- SKILL.md`.
