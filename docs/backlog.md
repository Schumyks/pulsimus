...
- **Tiling por cuadrantes (idea de Alan, 24/07 — dónde encaja):** dividir el composite en una grilla NO es método de MAPEO — un objeto cruza varias celdas → se fragmenta → re-fusionar es connected-components pero peor (ver [research](direccion/tecnicas-mapeo-verificacion-research.md) § 1d). PERO el tiling SÍ vive acá: partir un asset 4K/8K en tiles para **no reventar los 8GB de VRAM** al correr SAM (+ localización del peor error por celda, que Hausdorff ya da con su coordenada). Técnica de memoria/GPU, no de segmentación.
